...
ply_data_dir = os.path.join(BASE_DIR, './objs')
gpu_to_use = 0
...
test_file_list = os.path.join(BASE_DIR, 'testing_file_yujing.txt')
...
# Qi 2018.01.08
def load_pts_files(pts_file, catid):
    with open(pts_file, 'r') as f:
        pts_str = [item.rstrip() for item in f.readlines()]
        pts = np.array([np.float32(s.split()) for s in pts_str], dtype=np.float32)
    return pts
...
def predict():
    ...
    with tf.Session(config=config) as sess:
	    ...
	    pts = load_pts_files(pts_file_to_load, objcats[cur_gt_label])
	    ori_point_num = len(pts)
            ...
            if output_verbose:
                ...
                output_color_point_cloud(pts, seg_pred_val, os.path.join(output_dir, str(shape_idx)+pts_files[shape_idx][:-4]+'_pred.obj'))
